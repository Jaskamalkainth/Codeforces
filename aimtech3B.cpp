...
int main()
{
	...
	if(n == 2)
	{
		cout << min(abs(a-inp[0]),abs(a-inp[1]));
		return 0;
	}
	// 4 cases
	int ans = mx_int-1;
	ans = min(ans , 2*(a-inp[0]) + max(0,inp[n-2]-a));
	ans = min(ans , 2*(inp[n-1]-a) + max(0,a-inp[1]));
	ans = min(ans , 2*max(0,a-inp[1]) + inp[n-1]-a );
	ans = min(ans , 2*max(0,inp[n-2]-a) + a-inp[0]);
