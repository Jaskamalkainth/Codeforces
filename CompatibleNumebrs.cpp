...
int main()
{
	...
	for(int mask = 0; mask < (1<<N); mask++)
	{
		if(mask&1)
		{
			...
			temp[mask][0] = max(temp[mask][0] , temp[mask^1][0]);  
		}
		else
		{
			dp[mask][0] = cnt[mask]; 
			temp[mask][0] = temp[mask][0];
		}
		for(int i = 1; i < N; i++)
		{
			if(mask&(1<<i))
			{
				...
				temp[mask][i] = max(temp[mask][i-1] , temp[mask^(1<<i)][i-1]);  
			}
			else
			{
	...
	for(int i = 0; i < n; i++)
	{
		if(cnt[a[i]] * dp[(1<<N)-1-a[i]][N-1] > 0)
		{
			fans[i] = temp[(1<<N)-1-a[i]][N-1];  
		}
		else
		{
			fans[i] = -1;  
